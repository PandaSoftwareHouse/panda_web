...
class ibt_rec_controller extends CI_Controller {
    ...
    public function pending_list()
    {
        ...
        {
            $data = array(
                'query' => $this->db->query("SELECT b.`LocFromName` AS sname,b.`LocFrom` AS scode,a.* FROM backend.`dc_pick_rec` a INNER JOIN backend.`dc_pick` b ON b.`RefNo` = a.`RefNo`  WHERE a.post_status = 0")
            );
            ...
            if(strpos($browser_id,"Windows CE"))
            ...
            else
                {
                    ...
                    $this->load->view('ibt_rec/pending_list', $data);
    ...
    {   
        if($this->session->userdata('loginuser')== true && $this->session->userdata('username') != '')
        {
            ...
            {
                $this->load->view('header');
                $this->load->view('ibt_rec/scan_ibt');
                $this->load->view('footer');                    
            }
        }
        else
    ...
    public function scan_ibt_result()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') !='')
        { 
            $result = $this->db->query("SELECT IF(b.`post_status`IS NULL,'0',b.`post_status`) AS posted,IF(b.`RefNo`IS NULL,'0','1') AS record,a.* FROM backend.dc_pick a LEFT JOIN backend.`dc_pick_rec` b ON b.`RefNo` = a.`RefNo` WHERE a.RefNo = '".$this->input->post('ibt_no')."'");
            if($result->num_rows() != 0)// got result
            {
                $ibt_Data = array(
                                      
                    'ibt_no' => $this->input->post('ibt_no'),
                    'sup_code' => $result->row('LocFrom'),
                    'sup_name' => $result->row('LocFromName'),
                    );
                $this->session->set_userdata($ibt_Data);

                if($result->row('posted')=='1')
                {
                    $this->session->set_flashdata('message', 'IBT already Closed : '.$this->input->post('ibt_no'));
                    redirect('ibt_rec_controller/scan_ibt');
                };

                if($result->row('expiry_date') < $this->date())
                {
                    $this->session->set_flashdata('message', 'IBT already Expired : '.$this->input->post('ibt_no'). "<br> Expiry date : ". $result->row('expiry_date'));
                        redirect('ibt_rec_controller/scan_ibt');
                };

                if($result->row('record') == '1')
                {
                    $this->session->set_flashdata('message', 'IBT No already exist in pending list : '.$this->input->post('ibt_no') );
                    redirect('ibt_rec_controller/scan_ibt');
                }
                else
                {
                    redirect('ibt_rec_controller/trans_add?ibt_no='.$this->input->post('ibt_no')."&sname=".$result->row('LocFromName'));
                };
            }
            else
            {
                $this->session->set_flashdata('message', 'IBT No not found : '.$this->input->post('ibt_no'));
                redirect('ibt_rec_controller/scan_ibt');
            }
        }
        else
    ...
    public function trans_add()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            $get_dc_pick = $this->db->query("SELECT * FROM backend.dc_pick a where a.refno = '".$_REQUEST['ibt_no']."'");
            $data = array(
                'TRANS_GUID' => $this->guid(),
                'RefNo' => $get_dc_pick->row('RefNo'),
                'DocDate' => $get_dc_pick->row('DocDate'),
                'DeliverDate' => date('Y-m-d'),
                'CREATED_AT' => $this->datetime(), 
                'CREATED_BY' => $_SESSION['username'],
                'UPDATED_AT' => $this->datetime(),
                'UPDATED_BY' => $_SESSION['username'],
                'link_guid' => $get_dc_pick->row('TRANS_GUID'),
            );
            $this->db->insert('backend.dc_pick_rec', $data);
            $get_trans_guid = $this->db->query("SELECT a.`TRANS_GUID` FROM dc_pick_rec a ORDER BY a.`CREATED_AT` DESC LIMIT 1")->row('TRANS_GUID');
            // import from dc_pick_child
            $this->db->query("INSERT INTO backend.`dc_pick_rec_c` 
            SELECT 
            REPLACE(UPPER(UUID()),'-','') AS child_guid, 
            '$get_trans_guid' AS trans_guid,
            b.line,
            b.`Itemcode`,
            '0' AS qty_receive,
            '0' AS qty_varience, 
            ...
            '0' AS scan_flaq,
            b.`CHILD_GUID` 
            FROM dc_pick a INNER JOIN dc_pick_child b ON a.`TRANS_GUID` = b.`TRANS_GUID` 
            WHERE a.`RefNo` = '".$_REQUEST['ibt_no']."';
            ");
            redirect('ibt_rec_controller/pending_list');   
        }
        else
        {
            redirect('main_controller');
    ...
    public function barcode_scan()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            $result = $this->db->query("SELECT b.`LocFrom`,b.`LocFromName`,a.* FROM dc_pick_rec a INNER JOIN dc_pick b ON a.`RefNo` = b.`RefNo` WHERE a.TRANS_GUID = '".$_REQUEST['guid']."'");
            //echo $this->db->last_query();die;
            $_SESSION['trans_guid'] = $_REQUEST['guid'];
            $_SESSION['sup_name'] = $result->row('LocFromName');
            $_SESSION['sup_code'] = $result->row('LocFrom');

            $data = array(
                'rec_list' => $this->db->query("SELECT b.`Description`,a.* FROM dc_pick_rec_c a INNER JOIN dc_pick_child b ON b.`CHILD_GUID` = a.link_guid_c WHERE a.`TRANS_GUID` = '".$_REQUEST['guid']."' and a.scan_flag = 1 ORDER BY a.line ASC "),
            );
            $browser_id = $_SERVER["HTTP_USER_AGENT"];
            if(strpos($browser_id,"Windows CE"))
                {
                    $this->load->view('WinCe/header');
                    $this->load->view('WinCe/greceive/barcode_scan');
                }
            else
                {
                    $this->load->view('header');
                    $this->load->view('ibt_rec/barcode_scan', $data);
                    $this->load->view('footer');
                }    
        }
        else
    ...
    public function barcode_scan_result()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $result_itemmaster = $this->db->query("SELECT a.itemcode,a.itemlink,b.barcode,b.bardesc,a.packsize from backend.itemmaster a inner join backend.itembarcode b on a.itemcode=b.itemcode where barcode='".$this->input->post('barcode_scan')."'");
            $result = $this->db->query("SELECT * FROM dc_pick_rec_c a WHERE a.`TRANS_GUID` = '".$_SESSION['trans_guid']."' and barcode = '".$this->input->post('barcode_scan')."' ");

            if($result_itemmaster->num_rows() == 0)
            {
                $this->session->set_flashdata('message', 'Barcode Not Found : '.$this->input->post('barcode_scan'));
                redirect('ibt_rec_controller/barcode_scan?guid='.$_SESSION['trans_guid']);
            };

            if($result->num_rows() == 0)
            {
                $this->session->set_flashdata('message', 'Barcode Not Exist In IBT List : '.$this->input->post('barcode_scan'));
                redirect('ibt_rec_controller/barcode_scan?guid='.$_SESSION['trans_guid']);
            };

            if($result->row('Qty_Varience') <= '0' && $result->row('scan_flag') == '1')
            {
                $this->session->set_flashdata('message', 'PO BALANCE IS = ' .$result->row('Qty_Varience'). "<br>Unable To Scan : " .$result->row('description'));
                redirect('ibt_rec_controller/barcode_scan?guid='.$_SESSION['trans_guid']);
            };

            redirect('ibt_rec_controller/item_entry?itemcode='.$result->row('Itemcode'));
        }
        else
        {
            redirect('main_controller');
    ...
    {
        $ibt_details = $this->db->query("SELECT IF(a.`scan_flag` = 0,'ADD','EDIT') AS rec_status,b.`Description`,b.`Qty_Actual`,a.* FROM dc_pick_rec_c a INNER JOIN dc_pick_child b ON a.link_guid_c = b.`CHILD_GUID`WHERE a.`TRANS_GUID`= '".$_SESSION['trans_guid']."' AND a.itemcode = '".$_REQUEST['itemcode']."'");
        $_SESSION['CHILD_GUID'] = $ibt_details->row('CHILD_GUID');
        $data = array(
            'ibt_details' => $ibt_details,
            'set_master_code' => $this->db->query("SELECT * FROM backend.set_master_code WHERE trans_type='GRN_REASON' ORDER BY code_desc"),
            'reason' => $ibt_details->row('reason'),
            );
        $browser_id = $_SERVER["HTTP_USER_AGENT"];
        if(strpos($browser_id,"Windows CE"))
        {
            $this->load->view('WinCe/header');
            $this->load->view('WinCe/greceive/item_entry', $data);
        }
        else
        {
            $this->load->view('header');
            $this->load->view('ibt_rec/item_entry', $data);
            $this->load->view('footer');
        }
    }
    ...
    public function item_entry_update()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            if($this->input->post('rec_qty')-$this->input->post('actual_qty') < 0 && $this->input->post('reason') == '')
            {
                ...
                redirect('ibt_rec_controller/item_entry?itemcode='.$_REQUEST['itemcode']);
            }

            $data= array(
                'Qty_Received' => $this->input->post('rec_qty'), 
                'Qty_Varience' => $this->input->post('actual_qty') - $this->input->post('rec_qty'),
                'scan_flag' => 1,
                'UPDATED_AT' => $this->datetime('datetime'),
                'UPDATED_BY' =>  $_SESSION['username'],
                'reason'=> $this->input->post('reason'),
                'expiry_date'=> $this->input->post('expiry_date'),
                );
            $this->db->WHERE('CHILD_GUID',$_SESSION['CHILD_GUID']);
            $this->db->update('backend.dc_pick_rec_c',$data);
            redirect('ibt_rec_controller/barcode_scan?guid='.$_SESSION['trans_guid']);
        }
        else
        {
    ...
    public function post()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            $check_for_scan = $this->db->query("SELECT * FROM dc_pick_rec_c a where a.TRANS_GUID = '".$_REQUEST['guid']."' and a.scan_flag = '0' ");
            if($check_for_scan->num_rows() > 0)
            {
                $this->session->set_flashdata('message', 'Posting not allowed. Please make sure all item has been scan.'.$this->input->post('barcode_scan'));
                redirect('ibt_rec_controller/barcode_scan?guid='.$_REQUEST['guid']);
            }
        }
        else
        {
    ...
    public function item_entry_edit()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_high_shrink = $this->db->query("SELECT * FROM backend.`itemmaster_miscellaneous` WHERE itemcode = '".$batch_item->row('scan_itemcode')."' AND text1 = 'HIGH SHRINK' ");
    ...
    public function item_entry_add()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_high_shrink = $this->db->query("SELECT * FROM backend.`itemmaster_miscellaneous` WHERE itemcode = '".$_SESSION['scan_itemcode']."' AND text1 = 'HIGH SHRINK' ");
            $check_enable_high_shrink = $this->db->query("SELECT check_high_shrink FROM backend.xsetup a ")->row('check_high_shrink');
            $check_user_group = $this->db->query("SELECT a.* FROM backend_warehouse.set_user a INNER JOIN backend_warehouse.set_user_group b ON a.`user_group_guid` = b.`user_group_guid` WHERE b.`group_name` = 'SUPERVISOR' AND a.`user_name` = '".$_SESSION['username']."' ");
            if($check_high_shrink->num_rows() > 0 && $check_enable_high_shrink == 1 && $check_user_group->num_rows() == 0)
            {
