...
class greceive_controller extends CI_Controller {
    ...
    public function po_list()
    {
        ...
        {
            $data['grn_by_weight_direct_post_grn'] = $this->db->query("SELECT grn_by_weight_direct_post_grn from backend.xsetup limit 1")->row('grn_by_weight_direct_post_grn');
    ...
    public function cancel_po()
    {
        if($this->session->userdata('loginuser')== true && $this->session->userdata('username') != '')
        {
            $check_exist_item = $this->db->query("SELECT c.* FROM backend_warehouse.d_grn a INNER JOIN  backend_warehouse.`d_grn_batch` b ON a.`grn_guid` = b.`grn_guid`INNER JOIN backend_warehouse.`d_grn_batch_item` c ON c.`batch_guid` = b.`batch_guid`WHERE a.`grn_guid` = '".$_REQUEST['grn_guid']."'");

            if($check_exist_item->num_rows() > 0)
            {
                $this->session->set_flashdata('message', 'Item exist in this grn. Cancel not allowed.');
            ...
            else
            {
                ...
                $this->db->where('grn_guid', $_SESSION['grn_guid']);
                $this->db->update('backend_warehouse.d_grn', $data);
                $this->session->set_flashdata('message', 'Cancel Succesful.');
                redirect('greceive_controller/po_list');
            }
        }
        else
    ...
    public function scan_po_result()
    {
        ...
        { 
            ...
            $check_po_no = $this->greceive_model->check_po_no($po_no);
            if($check_po_no->num_rows() == 1)
            {
                ...
                {
                    $this->session->set_flashdata('message', 'PO No not posted : '.$po_no);
                    redirect('greceive_controller/scan_po');
                }
            }
            else
    ...
    public function scan_multiple_po_result()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            $po_no = $this->input->post('po_no');

            $check_first_po = $this->db->query("SELECT * FROM backend_warehouse.d_grn a WHERE a.grn_guid = '".$_REQUEST['grn_guid']."' ");
            $po_result = $this->greceive_model->scan_po_result($po_no);
            $po_child = $this->db->query("SELECT a.* FROM backend.pochild  AS a
                INNER JOIN (
                SELECT a.*, b.itemcode, b.`pricetype` FROM backend_warehouse.d_grn AS a
                INNER JOIN backend_warehouse.d_grn_po AS b
                ON a.grn_guid = b.grn_guid  
                WHERE a.grn_guid = '".$_REQUEST['grn_guid']."'
                /* to check first grn_guid scanned */
                AND pricetype = 'Norm'
                ) AS b
                ON a.itemcode = b.itemcode
                WHERE a.refno = '$po_no'
                /* to check second based on refno scanned */
                AND a.`PriceType` <> 'FOC'");
            $exist_record = $this->greceive_model->po_add($po_no);

            if($po_result->row('RefNo') == $check_first_po->row('po_no'))
            {
                $this->session->set_flashdata('message', 'Same PO Not Allowed : '.$po_no);
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };

            if($po_result->row('SCode') <> $check_first_po->row('scode'))
            {
                $this->session->set_flashdata('message', 'PO Not in same supplier'.$po_no);
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };

            if($po_result->row('Completed')=='1')
            {
                $this->session->set_flashdata('message', 'PO already Closed : '.$po_no);
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };

            if($po_result->row('expiry_date') < date('Y-m-d') )
            {
                $this->session->set_flashdata('message', 'PO already Expired : '.$po_no. "<br> Expiry date : ". $po_result->row('expiry_date'));
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };

            if($exist_record->num_rows() == 1)
            {
                $this->session->set_flashdata('message', 'PO No already exist in pending grn : '.$po_no );
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };

            if($po_child->num_rows() > 0)
            {
                $this->session->set_flashdata('message', 'Duplicate item exist. Process not allowed'.$po_no );
                redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
            };


            $this->session->set_flashdata('message_add', 'Add PO: '.$po_no.' to this GRN: '.$check_first_po->row('grn_id')."<br><br><a href='add_multiple_po?grn_guid=".$_REQUEST['grn_guid'].'&po_no='.$po_no."' class ='btn btn-primary btn-xs'>ADD</a>");
           redirect('greceive_controller/scan_po?multiple_po&grn_guid='.$_REQUEST['grn_guid']);
        }
        else
        {
    ...
    public function add_multiple_po()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $data = array(
                ...
                'grn_id' => $check_first_po = $this->db->query("SELECT * FROM backend_warehouse.d_grn a WHERE a.grn_guid = '".$_REQUEST['grn_guid']."' ")->row('grn_id'),       
                'po_no' => $_REQUEST['po_no'],        
                'po_date' => $this->greceive_model->scan_po_result( $_REQUEST['po_no'])->row('PODate'),      
            ...
            $this->db->insert("backend_warehouse.d_grn_multi_po",$data);
            $this->db->query("Call backend_warehouse.d_grn_create_podetail('".$_REQUEST['grn_guid']."','".$_REQUEST['po_no']."')");
            $this->db->query("Call backend_warehouse.d_grn_create_posum('".$_REQUEST['grn_guid']."')");
            $this->session->set_flashdata('message', 'Successful'. $_REQUEST['po_no'] );
            redirect('greceive_controller/po_list');
        }
        else
        {
    ...
    public function po_edit()
    {
        ...
        {
            $po_no = $_REQUEST['po_no'];
            $sname = $this->db->query("SELECT s_name from backend_warehouse.d_grn where po_no = '$po_no'")->row('s_name');
            /* added by hugh because if supplier name got &, it only CUT before &
            2018-04-16*/
            //$sname = $_REQUEST['sname'];

            $check_setup_allow_change_receiving_date = $this->db->query("SELECT a.`allow_change_receiving_date` FROM backend.`xsetup` a ")->row('allow_change_receiving_date');
            if($check_setup_allow_change_receiving_date == 0)// not allowed
            {
                $max_invoice_date = $this->greceive_model->po_details($po_no)->row('inv_date');
                $max_received_date = $this->greceive_model->po_details($po_no)->row('received_date');
            }
            else
            {
                $max_invoice_date = $this->greceive_model->po_details($po_no)->row('received_date');
                $max_received_date = '';
            }

            if(isset($_REQUEST['amt_exc_tax']))
            {
                $amt_exc_tax = $_REQUEST['amt_exc_tax']; 
                $gst_tax = $_REQUEST['gst_tax'];
                $rounding_adj = $_REQUEST['rounding_adj'];
                $amt_inc_tax = $_REQUEST['amt_inc_tax'];
                $do_no = $_REQUEST['do_no'];
                $inv_no = $_REQUEST['inv_no'];
            }
            else
            {
                $amt_exc_tax = $this->greceive_model->po_details($po_no)->row('amt_exc_tax'); 
                $gst_tax = $this->greceive_model->po_details($po_no)->row('gst_tax');
                $rounding_adj = $this->greceive_model->po_details($po_no)->row('rounding_adj');
                $amt_inc_tax = $this->greceive_model->po_details($po_no)->row('amt_inc_tax');
                $do_no = $this->greceive_model->po_details($po_no)->row('do_no');
                $inv_no = $this->greceive_model->po_details($po_no)->row('inv_no');
            ...
            $data = array(
                ...
                'sname' => $sname,
                'method' => 'Edit',
                'grn_by_weight_hide_inv_detail' => $this->db->query("SELECT grn_by_weight_hide_inv_detail from backend.xsetup ")->row('grn_by_weight_hide_inv_detail'),
    ...
    public function po_edit_update()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            if($grn_by_weight_hide_inv_detail == '0')
            {
                if($this->input->post('do_no') == '')
                {
                    $this->session->set_flashdata('message', 'Please key in D/O No.');
                    redirect('greceive_controller/po_edit?po_no='.$_SESSION['po_no']."&sname=".$_SESSION['sname']."&grn_guid=".$_SESSION['grn_guid'].'&amt_exc_tax='.$this->input->post('amt_exc_tax').'&gst_tax='.$this->input->post('gst_tax').'&rounding_adj='.$this->input->post('rounding_adj').'&amt_inc_tax='.$this->input->post('amt_inc_tax').'&inv_no='.$this->input->post('inv_no').'&do_no='.$this->input->post('do_no'));
                };
                ...
                if(round($this->input->post('amt_exc_tax')+$this->input->post('gst_tax')+$this->input->post('rounding_adj'),4) != round($this->input->post('amt_inc_tax'),4)  )
                {   
                ...
                $rounding_adj = $this->input->post('rounding_adj');
    ...
    public function po_add()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            if($check_setup_allow_change_receiving_date == 0)// not allowed
            {
                $max_invoice_date = $this->db->query("SELECT date_format(now(), '%Y-%m-%d') as  masaskrg")->row('masaskrg');
                $max_received_date = $this->db->query("SELECT date_format(now(), '%Y-%m-%d') as  masaskrg")->row('masaskrg');
            }
            else
            {
                $max_invoice_date = $this->db->query("SELECT date_format(now(), '%Y-%m-%d') as  masaskrg")->row('masaskrg');
    ...
    public function po_add_insert()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            if($this->input->post('do_no') == '')
            {
                ...
                redirect('greceive_controller/po_add?po_no='.$_SESSION['po_no']."&sname=".$_SESSION['sname'].'&amt_exc_tax='.$this->input->post('amt_exc_tax').'&gst_tax='.$this->input->post('gst_tax').'&rounding_adj='.$this->input->post('rounding_adj').'&amt_inc_tax='.$this->input->post('amt_inc_tax').'&inv_no='.$this->input->post('inv_no').'&do_no='.$this->input->post('do_no'));
            };
            ...
            /*$getRefNo = $this->db->query("SELECT CONCAT(run_code, REPLACE(run_date, '-', ''), REPEAT(0,run_digit-LENGTH(run_currentno + 1)), run_currentno,LPAD(FLOOR(RAND() * 99),2,0))
                ...
                // error when running no turn into 9

                $getRefNo = $this->db->query("SELECT CONCAT(run_code, REPLACE(run_date, '-', ''), LPAD(run_currentno,run_digit,0),LPAD(FLOOR(RAND() * 99),2,0))
                AS refno FROM backend_warehouse.set_sysrun WHERE run_type = 'GRNBATCH'");
    ...
    public function po_batch()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $doc_posted= $_REQUEST['doc_posted'];
        ...
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            else
            {
                ...
                $query = $this->db->query("SELECT * from backend_warehouse.d_grn_posum 
                    where grn_guid='$grn_guid' and itemlink = '$itemlink'
                     and itemcode = '".$result->row('itemcode')."' ORDER by grn_diff DESC ");

                /*$query = $this->db->query("SELECT * from backend_warehouse.d_grn_posum 
                    where grn_guid='$grn_guid' and itemlink = '$itemlink'
                     and itemcode = '".$result->row('itemcode')."' ORDER by grn_diff DESC ");*/
                // close solve packsize issue
                if($query->num_rows() == 0)
                {
                    $query_itemlink = $this->db->query("SELECT * from backend_warehouse.d_grn_posum where grn_guid='$grn_guid' and itemlink = '$itemlink' ORDER by grn_diff DESC");
                    if($query_itemlink->num_rows() > 0)
                    {
                        redirect('greceive_controller/have_itemlink?posum_guid='.$query_itemlink->row('posum_guid')."&itemlink=".$query_itemlink->row('itemlink'));
                    }
                    else
                    {
    ...
    public function have_focitem()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $query2 = $this->db->query("SELECT itemcode,itemlink,description,packsize,foc_qty,po_qty,posum_guid from backend_warehouse.d_grn_posum where grn_guid='".$_SESSION['grn_guid']."' and itemlink='".$query->row('itemlink')."' AND itemcode = '$itemcode' order by po_qty desc");
    ...
    public function have_itemlink()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_match_po = $this->db->query("SELECT a.*FROM backend.itemmaster a INNER JOIN backend_warehouse.d_grn_posum b ON a.`Itemcode` = b.`itemcode` WHERE a.ItemLink='$itemlink' AND b.itemcode = '$check_itemcode' ORDER BY packsize;");
            // $check_setup_for_choose_packsize = $this->db->query("SELECT choose_packsize_for_receiving FROM backend.`xsetup` a WHERE a.`choose_packsize_for_receiving` = '1'")->row('choose_packsize_for_receiving');
            // cmartdc requirement
            ...
            if($check_match_po->num_rows() > 0)// && $check_setup_for_choose_packsize == 0
            {
                redirect('greceive_controller/item_entry_add?scan_itemcode='.$check_itemcode);
            };
    ...
    public function po_print()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            if ($_REQUEST['print_type']=='batch_only')
            {
                ...
                redirect('greceive_controller/po_batch?grn_guid='.$_SESSION['grn_guid']."&po_no=".$_SESSION['po_no']."&sname=".$_SESSION['sname']."&scode=".$_SESSION['scode']."&doc_posted=".$_SESSION['doc_posted']);
        ...
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            if($grn_by_weight == 0)
            {
                ...
                if($rec_count == 0)
                {
                    ...
                    // check supcus if supplier in auto grn supplier po == 

                    $check_query_grn_supplier = $this->db->query("SELECT *  FROM backend_warehouse.d_grn WHERE grn_guid='$grn_guid'");
                    $get_scode =  $check_query_grn_supplier->row('scode');
                    $check_auto_grn_supplier_po = $this->db->query("SELECT * from backend.supcus where code = '$get_scode'")->row('auto_grn_mobile_po');

                    if($check_auto_grn_supplier_po == '1')
                    {
                        //comeback here
                        $get_po_no = $check_query_grn_supplier->row('po_no');
                        // get_grn_guid
                        $check_d_grn_po = $this->db->query("SELECT * FROM backend_warehouse.d_grn_po WHERE po_no = '$get_po_no' group by po_line"); 
                        $get_grn_guid = $check_d_grn_po->row('grn_guid');

                        $datetime = $this->db->query("SELECT NOW() AS datetime");

                        $check_in_d_grn_batch_item = $this->db->query("SELECT * from backend_warehouse.d_grn_batch_item where batch_guid = '$batch_guid'");
                        //echo var_dump($check_in_d_grn_batch_item->num_rows());die;
                        if($check_in_d_grn_batch_item->num_rows() == '0')
                        {
                            foreach($check_d_grn_po->result() as $i => $id )
                            {
                            $barcode = $this->db->query("SELECT cost_code from backend.itemmaster where itemcode = '".$id->itemcode."'")->row('barcode');
                            $this->db->query("REPLACE INTO backend_warehouse.d_grn_batch_item 
                                SELECT 
                                upper(replace(uuid(),'-','')) as  item_guid   
                                , '$batch_guid' as batch_guid
                                , '".$id->po_line."' as lineno
                                , '".$id->itemcode."' as po_itemcode
                                , '".addslashes($id->description)."' as po_description
                                , '".$id->itemlink."' as po_itemlink
                                , '".$id->packsize."' as po_packsize
                                , '".$id->itemcode."' as scan_itemcode
                                , '".addslashes($id->description)."' as scan_description
                                , '".$id->itemlink."' as scan_itemlink
                                , '".$id->packsize."' as scan_packsize
                                , '' as scan_weight
                                , '".$id->po_qty."' as qty_do
                                , '".$id->po_qty."' as qty_rec
                                , '' as scan_weight_total
                                , '".$id->posum_guid."' as posum_guid
                                , '".$datetime->row('datetime')."' as created_at
                                , '' as created_by
                                , '".$datetime->row('datetime')."' as updated_at
                                , '' as updated_by
                                  , '' as WeightTraceQty          
                                  , '' as  WeightTraceQtyUOM       
                                  , '' as  WeightTraceQtyCount     
                                  , '' as  PurTolerance_Std_plus   
                                  , '' as  PurTolerance_Std_Minus  
                                  , '' as  scan_as_itemcode        
                                  , '$barcode' as  scan_barcode            
                                  , '' as  qty_diff_is_foc         
                                  , '' as  qty_diff                
                                  , '' as  stock_update            
                                  , '' as  stock_update_at         
        ...
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            if($_SESSION['grn_by_weight'] == '0')// others client
            {
                $data = array(
                    ...
                    'check_pay_by_invoice' => $this->db->query("SELECT grn_baseon_pocost FROM supcus WHERE `code` = '".$_SESSION['scode']."'")->row('grn_baseon_pocost'),
                );
            }
            else// midas client
            {
    ...
    public function item_entry_edit()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $_SESSION['barcode'] = $batch_item->row('scan_barcode');

            $check_high_shrink = $this->db->query("SELECT * FROM backend.`itemmaster_miscellaneous` WHERE itemcode = '".$batch_item->row('scan_itemcode')."' AND misc_group = 'REQUIRE SUPERVISION @ GR' and set_active = 1 ");
            $check_enable_high_shrink = $this->db->query("SELECT check_high_shrink FROM backend.xsetup a ")->row('check_high_shrink');
            // $check_user_group = $this->db->query("SELECT a.* FROM backend_warehouse.set_user a INNER JOIN backend_warehouse.set_user_group b ON a.`user_group_guid` = b.`user_group_guid` WHERE b.`group_name` = 'SUPERVISOR' AND a.`user_name` = '".$_SESSION['username']."' ");
            $check_user_group = $this->db->query("SELECT * from backend_warehouse.set_user a inner join backend_warehouse.`set_user_group` b on b.`user_group_guid` = a.`user_group_guid` inner join backend_warehouse.`set_user_group_webmodule` c on c.`user_group_guid` = b.`user_group_guid` where a.`user_name` = 'panda' and c.`module_name` = 'Athorise High Shrink Item';");

            if($check_high_shrink->num_rows() > 0 && $check_enable_high_shrink == 1 && $check_user_group->num_rows() == 0)
            {
                $superid_required = 1;
            }
            else
            ...
            $data = array(
                ...
                'superid_required' => $superid_required,
            ...
            if(isset($_SESSION['method']) && $_SESSION['method'] == 'by_batch' && $_SESSION['edit_item'] == 0)
            {
                ...
                $data_insert = array(
                    ...
                    'lineno' => $this->db->query("SELECT COUNT(*)+1 AS reccount FROM backend_warehouse.d_grn_batch_item_c WHERE item_guid = '".$_SESSION['item_guid']."' ")->row('reccount'),
    ...
    public function item_entry_update()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_supervisor = $this->db->query("SELECT a.* FROM backend_warehouse.set_user a INNER JOIN backend_warehouse.set_user_group b ON a.`user_group_guid` = b.`user_group_guid` WHERE b.`group_name` = 'SUPERVISOR' AND a.`user_password` = '".$this->input->post('superid')."' ");
            if($check_supervisor->num_rows() == 0 && $this->input->post('superid_required') == 1)
            {
                $this->session->set_flashdata('message', '<span class="label label-danger" style="font-size: 14px;">Incorrect supervisor id.</span>');
            ...
            $check_expiry_setup = $this->db->query("SELECT * FROM backend.supcus a WHERE a.`Code` = '".$_SESSION['scode']."'; ")->row('min_expiry_day');
            $check_interval_days = $this->db->query("SELECT CURDATE() + INTERVAL (SELECT a.`value1` AS days FROM backend.`itemmaster_miscellaneous` a WHERE a.misc_group = 'MIN EXPIRY DAY' AND a.`itemcode` = '".$batch_item->row('scan_itemcode')."') DAY AS expected_expiry_date;")->row('expected_expiry_date');
            $check_misc_active = $this->db->query("SELECT a.set_active AS set_active FROM backend.`itemmaster_miscellaneous` a WHERE a.misc_group = 'MIN EXPIRY DAY' AND a.`itemcode` = '".$batch_item->row('scan_itemcode')."' ")->row('set_active');

            // check based on supplier setup and check expected expired date greater than input expired date
            if($check_expiry_setup == 1 && $check_interval_days > $this->input->post('expiry_date') && $check_misc_active == 1)
            {
                $this->session->set_flashdata('message', '<span class="label label-warning" style="font-size: 14px;">Expiry date should be greater than :'.$check_interval_days.' </span>');
                redirect('greceive_controller/item_entry_edit?edit_item&item_guid='.$item_guid.'&batch_guid='.$batch_guid.'&posum_guid='.$posum_guid);
            }
    ...
    public function item_entry_add()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_high_shrink = $this->db->query("SELECT * FROM backend.`itemmaster_miscellaneous` WHERE itemcode = '".$_SESSION['scan_itemcode']."' AND text1 = 'REQUIRE SUPERVISION @ GR' and set_active = 1 ");
            $check_enable_high_shrink = $this->db->query("SELECT check_high_shrink FROM backend.xsetup a ")->row('check_high_shrink');
            $check_user_group = $this->db->query("SELECT a.* FROM backend_warehouse.set_user a INNER JOIN backend_warehouse.set_user_group b ON a.`user_group_guid` = b.`user_group_guid` WHERE b.`group_name` = 'SUPERVISOR' AND a.`user_name` = '".$_SESSION['username']."' ");
...
echo var_dump($batch_item->num_rows());die;here*/
            ...
            if ($_SESSION['recqty_aspoqty'] == 1 && $_SESSION['redirected'] == '0')
            {
               $received_qty = $_SESSION['po_packsize']/$new_record->row('packsize')*($grn_posum->row('po_qty')+$grn_posum->row('foc_qty'));
            }
            elseif($_SESSION['recqty_aspoqty'] == 1 && $_SESSION['redirected'] == '1')
            {
                $received_qty =  $_SESSION['rec_qty'];
            }
            elseif(isset($_REQUEST['received_qty']))
            {
            ...
            if ($_SESSION['doqty_aspoqty'] == 1 && $_SESSION['redirected'] == '0')
            {
               $do_qty = $_SESSION['po_packsize']/$new_record->row('packsize')*($grn_posum->row('po_qty')+$grn_posum->row('foc_qty'));
            }
            elseif($_SESSION['doqty_aspoqty'] == 1 && $_SESSION['redirected'] == '1')
            {
                $do_qty =  $_SESSION['do_qty'];
            }
            elseif(isset($_REQUEST['do_qty']))
            {
            ...
            if(isset($_REQUEST['redirect_received_qty']))
            {
                $redirect_received_qty = $_REQUEST['redirect_received_qty'];
            }
            else
            {
                $redirect_received_qty = '0';
            }
            ...
            $data = array(
                ...
                'redirect_received_qty' => $redirect_received_qty,
                ...
                'check_bulk_qty' => $this->db->query("SELECT a.bulkqty FROM itemmaster a INNER JOIN itembarcode b ON a.itemcode = b.itemcode WHERE itemlink = '".$_SESSION['itemlink']."' AND Barcode = '".$_SESSION['barcode']."' AND a.itemcode = '".$_REQUEST['scan_itemcode']."' ")->row('bulkqty'),

                'bulk_qty' => $this->db->query("SELECT CONCAT(a.bulkqty, ' ', a.umbulk) AS bulkqty FROM itemmaster a INNER JOIN itembarcode b ON a.itemcode = b.itemcode WHERE itemlink = '".$_SESSION['itemlink']."' AND Barcode = '".$_SESSION['barcode']."' AND a.itemcode = '".$_REQUEST['scan_itemcode']."' ")->row('bulkqty'),
    ...
    public function item_entry_insert()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            $check_expiry_setup = $this->db->query("SELECT * FROM backend.supcus a WHERE a.`Code` = '".$_SESSION['scode']."'; ")->row('min_expiry_day');
            $check_interval_days = $this->db->query("SELECT CURDATE() + INTERVAL (SELECT a.`value1` AS days FROM backend.`itemmaster_miscellaneous` a WHERE a.misc_group = 'MIN EXPIRY DAY' AND a.`itemcode` = '".$_SESSION['scan_itemcode']."') DAY AS expected_expiry_date;")->row('expected_expiry_date');
            $check_misc_active = $this->db->query("SELECT a.set_active AS set_active FROM backend.`itemmaster_miscellaneous` a WHERE a.misc_group = 'MIN EXPIRY DAY' AND a.`itemcode` = '".$_SESSION['scan_itemcode']."' ")->row('set_active');
            //echo $this->db->last_query();die;
            ...
            $_SESSION['do_qty'] = $do_qty;
            
            $_SESSION['rec_qty'] = $rec_qty;
           
            $check_supervisor = $this->db->query("SELECT a.* FROM backend_warehouse.set_user a INNER JOIN backend_warehouse.set_user_group b ON a.`user_group_guid` = b.`user_group_guid` WHERE b.`group_name` = 'SUPERVISOR' AND a.`user_password` = '".$this->input->post('superid')."' ");
            if($check_supervisor->num_rows() == 0 && $this->input->post('superid_required') == 1)
            {
                $this->session->set_flashdata('message', '<span class="label label-danger" style="font-size: 14px;">Incorrect supervisor id.</span>');
                redirect('greceive_controller/item_entry_add?scan_itemcode='.$this->input->post('scan_itemcode').'&redirect_received_qty='.$this->input->post('rec_qty'));
            }
            ...
            if($rec_qty-$balance_qty < 0 && $reason == '')
            {
                ...
                $this->session->set_flashdata('message', '<span class="label label-warning" style="font-size: 14px;">Variance Qty Exist. Please Select Reason. </span>');
                    redirect('greceive_controller/item_entry_add?scan_itemcode='.$this->input->post('scan_itemcode').'&redirect_received_qty='.$this->input->post('rec_qty').'&do_qty='.$do_qty.'&received_qty='.$rec_qty);
            }

            // if($rec_qty-$balance_qty > 0 && $reason == '')
            if($rec_qty > $balance_qty+$foc_qty && $reason == '')
            {
                $_SESSION['redirected'] = '1';
                $this->session->set_flashdata('message', '<span class="label label-warning" style="font-size: 14px;">Qty input is MORE than PO. Please Select Reason. </span>');
                    redirect('greceive_controller/item_entry_add?scan_itemcode='.$this->input->post('scan_itemcode').'&redirect_received_qty='.$this->input->post('rec_qty').'&do_qty='.$do_qty.'&received_qty='.$rec_qty);
            }
    ...
    public function convert_excess_to_foc()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            else
            {
                ...
                if($batch_item->row('qty_diff') > 0 )
                {
                    redirect('greceive_controller/convert_excess_to_foc_1?item_guid='.$item_guid.'&qty_diff='.$batch_item->row('qty_diff'));
                }
                redirect('greceive_controller/barcode_scan?by_qty&'.$batch_item->row('qty_diff'));  
            }
        }
    ...
    public function convert_excess_to_foc_1()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            ...
            // echo $this->db->last_query();die;
            // redirect('greceive_controller/barcode_scan');
            $this->session->set_flashdata('message_update_success', 'PO BALANCE IS = '.$_REQUEST['qty_diff']);
            redirect('greceive_controller/barcode_scan?by_qty&'.$_REQUEST['qty_diff']);
    ...
    public function search_gr_result()
    {
        ...
        {   
            ...
            {
                $refno = $this->db->query("SELECT a.RefNo,a.Name,b.* from backend.grmain as a inner join backend_warehouse.d_grn  as b on a.refno = b.convert_grn_by where a.refno = '".$_REQUEST['gr_no']."' and b.po_no = '".$_REQUEST['po_no']."' and b.canceled <> 1");
            }
            else
            {
                // make sure that d_grn and grmain got the record
                $refno = $this->db->query("SELECT a.RefNo,a.Name,b.* from backend.grmain as a inner join backend_warehouse.d_grn  as b on a.refno = b.convert_grn_by where a.refno = '$gr_no' and b.canceled <> 1");
            }
            /*echo var_dump($_SESSION);
            echo $this->db->last_query(); die;*/
            if($refno->num_rows() == 0)
    ...
    public function update_d_grn_grmain()
    {
        ...
        {   
            ...
            $check_current_record = $this->db->query("SELECT * from backend_warehouse.d_grn where convert_grn_by = '".$_SESSION['gr_no']."'");
            ...
            {
                $this->session->set_flashdata('message', 'Do No cannot be empty');
                redirect('greceive_controller/search_gr_result?gr_no='.$_SESSION['gr_no']);
            };
            /*if($this->input->post('inv_date') < $this->input->post('received_date'))
            ...
            $this->greceive_model->d_grn_update($data);
    ...
    public function outstanding_po()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        { 
            if(isset($_REQUEST['localdate']))
            {
                $selected_date = $_REQUEST['localdate'];   
            }
            else
            {
            ...
            if(isset($_REQUEST['localdate']) == '1' && $_REQUEST['localdate'] == '1')
            {
                 $ospo = $this->db->query("SELECT refno, sname, scode, podate, deliverdate from backend.pomain where completed <> '1' order by sname asc, refno asc ");    
            }
            elseif(isset($_REQUEST['localdate']) == '0')
            {
                $ospo = $this->db->query("SELECT refno, sname, scode, podate, deliverdate from backend.pomain where deliverdate = '$selected_date' and completed <> '1' order by sname asc, refno asc ");
            }
            else
            {
                 $ospo = $this->db->query("SELECT refno, sname, scode, podate, deliverdate from backend.pomain where deliverdate = '$selected_date' and completed <> '1' order by sname asc, refno asc ");
            ...
            if(strpos($browser_id,"Windows CE"))
                {
                    $this->load->view('WinCe/header');
                    $this->load->view('WinCe/greceive/os_list');
                }
            else
                {
                    $this->load->view('header');
                    $this->load->view('greceive/os_list',$data);
                    $this->load->view('footer');                    
                }
    ...
    public function outstanding_po_child_detail()
    {
        if($this->session->userdata('loginuser') == true && $this->session->userdata('username') != '')
        {
            $refno = $_REQUEST['refno'];

            $pomain = $this->db->query("SELECT * from pomain where refno = '$refno'");
            $pochild = $this->db->query("SELECT * from pochild where refno = '$refno'");

            $data  = array (
                'pomain' => $pomain,
                'pochild' => $pochild,
                'backbutton' => site_url("greceive_controller/outstanding_po?localdate=".$_SESSION['localdate']),
            );

            $this->load->view('header');
            $this->load->view('greceive/os_list_c',$data);
            $this->load->view('footer'); 
    ...
    public function posted_doc()
    {
        ...
        { 
            ...
            $posted_doc = $this->db->query("SELECT 1 AS Seq,a.* FROM backend_warehouse.d_grn a WHERE received=1 AND 
        trans_type='GRN_WEIGHT' AND created_by='$username' AND sublocation='$sublocation' AND canceled=0 AND left(received_at, 10) = '$selected_date'
        UNION ALL 
        SELECT 2 AS Seq,a.* FROM backend_warehouse.d_grn a WHERE received=1 AND 
        trans_type='GRN_WEIGHT' AND created_by<>'$username' AND 
        sublocation='$sublocation' AND canceled=0 and left(received_at, 10) = '$selected_date' ORDER BY Seq,created_by, created_at DESC");
            //echo $this->db->last_query();die;
            $data = array(
                'posted_doc' => $posted_doc,
                'selected_date' => $selected_date,
            );
            ...
            if(strpos($browser_id,"Windows CE"))
                {
                    $this->load->view('WinCe/header');
                    $this->load->view('WinCe/greceive/posted_list');
                }
            else
                {
                    $this->load->view('header');
                    $this->load->view('greceive/posted_list',$data);
                    $this->load->view('footer');                    
                }
