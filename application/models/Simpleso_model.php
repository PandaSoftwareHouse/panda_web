...
class simpleso_model extends CI_Model
{
	...
	{
		$sql = "SELECT IF(b.item_guid IS NOT NULL ,'BATCH','QTY') 
 AS scan_type,b.scan_guid,b.item_guid,a.* FROM 
 (SELECT b.module_desc,a.* FROM web_trans_c a INNER JOIN web_trans b ON a.web_guid = b.web_guid 
 WHERE a.web_guid='$web_guid' ORDER BY created_at DESC) a LEFT JOIN (SELECT * FROM backend_warehouse.`d_batch_scan_log` a WHERE a.`type` = 'Simple SO')b ON a.itemcode = b.scan_itemcode GROUP BY a.itemcode ORDER BY a.created_at DESC";
		$query = $this->db->query($sql);
        return $query;
	}
	...
	public function add_process($remarks)
	{
		$location = $_SESSION['location'];
		$username = $_SESSION["username"];
		$sql = "INSERT INTO backend.web_trans (web_guid, module_desc, remarks, location, loc_group, created_at, created_by)
		VALUES ( REPLACE(UPPER(UUID()),'-',''), 'Simple SO', '$remarks', '$location', '$location', NOW(), '$username' )";
		$query = $this->db->query($sql);
        return $query;
	}
