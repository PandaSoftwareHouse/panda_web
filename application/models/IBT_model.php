...
class IBT_model extends CI_Model
{
	...
	public function main()
    {
		...
		module_desc= 'IBT Req' AND posted=0 AND cancel=0 ORDER BY created_at DESC";
    ...
    public function search_result($branchname)
    {
        //data is retrive from this query
        $sql = "SELECT branch_code AS CODE,branch_name AS NAME,CONCAT(branch_name,' ->(',branch_code,')') AS dname  
                FROM backend.cp_set_branch WHERE branch_name LIKE '%$branchname%' ORDER BY branch_code LIMIT 100";
        $query = $this->db->query($sql);
        return $query;
    ...
    public function add_trans($fcode,$tcode,$tname)
    {
        ...
        //data is retrive from this query
        $sql = "INSERT INTO backend.web_trans (web_guid, remarks, module_desc,acc_code,acc_name,location,loc_group,ibt_from,created_at,created_by,updated_at,updated_by)
        VALUES ( REPLACE(UPPER(UUID()),'-',''), '', 'IBT Req','$tcode','$tname','$fcode','$fcode','$fcode',NOW(),'$username',NOW(),'$username')";
        $query = $this->db->query($sql);
        return $query; 
    }


    public function item_in_IBT($web_guid)
    {
        //data is retrive from this query
...
LEFT JOIN (SELECT * FROM backend_warehouse.`d_batch_scan_log` a WHERE a.`type` = 'IBT Req')b ON a.itemcode = b.scan_itemcode 
GROUP BY a.itemcode ORDER BY a.created_at DESC";
        $query = $this->db->query($sql);
        return $query; 
    }

// load web_guid 2017-01-16
/*        public function check_webguid($web_guid)
    {
        //data is retrive from this query
...
// load web_guid 2017-01-16
    ...
    public function item_in_IBT_header($web_guid)
    {
        //data is retrive from this query
        $sql = "SELECT a.*, CONCAT ('$',FORMAT(bill_amt,2)) AS bill_amt_format FROM backend.web_trans a WHERE 
        module_desc= 'IBT Req' AND posted=0 AND cancel=0 AND web_guid='$web_guid' ORDER BY created_at DESC";
        $query = $this->db->query($sql);
        return $query; 
    }
