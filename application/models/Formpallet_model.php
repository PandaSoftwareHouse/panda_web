...
class formpallet_model extends CI_Model
{
    ...
    public function check_grn_guid($grn_guid)
    {
        $sql = "SELECT grn_guid from backend_warehouse.d_grn where grn_guid='$grn_guid' ";
    ...
    public function po_list()
    {
        ...
        trans_type='GRN_WEIGHT' AND created_by='$username' AND sublocation='$location' AND canceled=0 
        UNION ALL 
        ...
        sublocation='$location' AND canceled=0 ORDER BY Seq,created_by, created_at DESC";
        $query = $this->db->query($sql);
        return $query;
    ...
    public function m_batch($grn_guid)
    {
        //data is retrive from this query
        $sql = "SELECT * from backend_warehouse.d_grn_batch where stock='0' and grn_guid='$grn_guid' order by batch_id desc";
        $query = $this->db->query($sql);
        //echo $this->db->last_query();die;
        return $query;
    }
    ...
    public function m_post_stock_scan($batch_barcode)
    {
        $location = $_SESSION["location"];
        $sql = "SELECT * from backend_warehouse.d_grn_batch where stock='0' and batch_barcode='$batch_barcode' ";
        $query = $this->db->query($sql);
        //echo $this->db->last_query();die;
        return $query;
    }

    function update_m_post_stock_scan($batch_barcode, $data)
    {
        $this->db->where('batch_barcode', $batch_barcode);
    ...
    public function m_po_print_batch_only($batch_guid)
    {
        //data is retrive from this query
    ...
    public function d_grn_recal_batch_variance()
    {
        $batch_guid = $_SESSION['batch_guid'];
        $sql = "CALL backend_warehouse.d_grn_recal_batch_variance('$batch_guid')";
        $query = $this->db->query($sql);
    ...
    {
        //data is retrive from this query
        $sql = "SELECT * from backend_warehouse.d_grn_batch_item where batch_guid='$batch_guid' order by lineno desc";
        $query = $this->db->query($sql);
        return $query;
    }
    ...
    {
        $this->db->where('run_type', 'MANUAL_BATCH');
